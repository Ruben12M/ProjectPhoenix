if Config.VehicleKeys.ENABLE then
    ...
    AddEventHandler('cd_garage:AddKeys_playerload', function(data)
        ...
        for c, d in pairs(data) do
            AddKey(GetCorrectPlateFormat(d.plate))
        end
    end)
    ...
    if Config.VehicleKeys.Lockpick.ENABLE then
        ...
        TriggerEvent('chat:addSuggestion', '/'..Config.VehicleKeys.Lockpick.command, L('chatsuggestion_lockpick'))
        RegisterCommand(Config.VehicleKeys.Lockpick.command, function()
            TriggerEvent('cd_garage:LockpickVehicle')
        end)

        RegisterNetEvent('cd_garage:LockpickVehicle')
        AddEventHandler('cd_garage:LockpickVehicle', function()
            local vehicle = GetClosestVehicle(5)
            if vehicle then
                local plate = GetPlate(vehicle)
                CacheVehicle(plate, vehicle)
