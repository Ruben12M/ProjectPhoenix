    ...
    CreateThread(function()
        ...
        for k,v in pairs(Config.EvidenceLabs) do
            ...
            for k,v in pairs(points) do
                function v.analyze:nearby()
                    if not self.isClosest then return end
                    local authorized
                    if Config.EvidenceLabs[self.lab].jobLock and HasGroup(Config.EvidenceLabs[self.lab].jobLock) then authorized = true elseif not Config.EvidenceLabs[self.lab].jobLock then authorized = true end

                    if authorized and IsPolice and self.currentDistance < self.distance then
                        if not textUI then
                            ShowTextUI(Config.EvidenceLabs[self.lab].analyze.label)
                            textUI = true
                        end
                        if IsControlJustReleased(0, 38) then
                            AnalyzeEvidence(self.lab)
                        end
                    end
                end
                function v.analyze:onExit()
                    if not self.isClosest then return end
                    if textUI then
                        HideTextUI()
                        textUI = nil
                ...
                function v.storage:nearby()
                    if not self.isClosest then return end
                    local authorized
                    if Config.EvidenceLabs[self.lab].jobLock and HasGroup(Config.EvidenceLabs[self.lab].jobLock) then authorized = true elseif not Config.EvidenceLabs[self.lab].jobLock then authorized = true end
                    if authorized and IsPolice and self.currentDistance < self.distance then
                        if not textUI then
                            ShowTextUI(Config.EvidenceLabs[self.lab].storage.label)
                            textUI = true
                        end
                        if IsControlJustReleased(0, 38) then
                            AccessEvidenceStorage()
                ...
                function v.fingerprinting:nearby()
                    if not self.isClosest then return end
                    local authorized
                    if Config.EvidenceLabs[self.lab].jobLock and HasGroup(Config.EvidenceLabs[self.lab].jobLock) then authorized = true elseif not Config.EvidenceLabs[self.lab].jobLock then authorized = true end
                    if authorized and IsPolice and self.currentDistance < self.distance then
                        if not textUI then
                            ShowTextUI(Config.EvidenceLabs[self.lab].fingerprinting.label)
                            textUI = true
                        end
                        if IsControlJustReleased(0, 38) then
                ...
                function v.fingerprinting:onExit()
                    if not self.isClosest then return end
                    if textUI then
                        HideTextUI()
                        textUI = nil
