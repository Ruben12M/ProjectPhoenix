...
Config.wasabi_crutch = { -- If you use wasabi_crutch: https://store.wasabiscripts.com/category/2080869
    -- Crutch Settings
    crutchInMedbag = { enabled = false, item = 'crutch' }, -- Enabled? Item name? REQUIRES WASABI_CRUTCH
    ...
    -- Chair settings
    chairInMedbag = { enabled = false, item = 'wheelchair' },  -- Enabled? Item name? REQUIRES WASABI_CRUTCH
    chairInJobMenu = false -- Chair menu accessible from job menu if true. REQUIRES WASABI_CRUTCH
}

Config.phoneDistress = true -- Options: 'gks' (GKS Phone - ESX ONLY) / 'qs' (qs-smartphone) / 'd-p' (d-phone) WILL REPLACE BUILT IN DISPATCH WITH PHONE DISPATCH / Add additonal dispatch in client/cl_customize.lua
...
Config.skinScript = 'qb-clothing' -- Current options: 'esx' (For esx_skin) / 'appearance' (For wasabi-fivem-appearance) / 'qb' (For qb-clothing) / 'Custom' for any clothing script (editable in cl_customize.lua)
...
Config.Inventory = 'qs' --Options include: 'ox' - (ox_inventory) / 'qb' - (QBCore qb-inventory) 'mf' - (mf-inventory) / 'qs' (qs-inventory) / 'other' (whatever else can customize in client/cl_customize.lua)
...
Config.Locations = {
    Pillbox = {
        ...
        PersonalLocker = {
            enabled = true, -- Enable personal locker(stash) - THIS IS CURRENTLY ONLY AVALIABLE IN QBCORE
