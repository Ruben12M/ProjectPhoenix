...
AddEventHandler('wasabi_police:grantLicense', function()
    ...
    if tonumber(grade) < Config.GrantWeaponLicenses.minGrade then
        TriggerEvent('wasabi_police:notify', Strings.grade_to_low, Strings.rade_to_low_desc, 'error')
        return
    end
    local coords = GetEntityCoords(cache.ped)
...
AddEventHandler('wasabi_police:toggleDuty', function()
    ...
    TriggerServerEvent('wasabi_police:svToggleDuty', PlayerData.job.name, PlayerData.job.grade)
end)
            ...
            if v.clockInAndOut.target.enabled then
                ...
                CreateThread(function()
                    ...
                    function point:nearby()
                        ...
                        local jobName, jobGrade = HasGroup(self.job)
                        if jobName then
                            hasJob = jobName
                        elseif Framework == 'esx' then
                            jobName, jobGrade = HasGroup('off'..self.job)
                            if jobName then hasJob = jobName end
                        end
                        if hasJob then
                            if not textUI then
                                ShowTextUI(v.clockInAndOut.label)
                                textUI = true
                            end
                            if IsControlJustReleased(0, 38) then
                                if Framework == 'qb' then
...
CreateThread(function()
    ...
    while Config.GPSBlips do
        temp = lib.callback.await('wasabi_police:getPlayerCoords', 100)
        if not temp then goto skip end
        for k, data in pairs(temp) do
            blips[#blips + 1] = createBlip(vector(data.coords.x, data.coords.y, data.coords.z), Config.GPSBlips.blip.sprite, Config.GPSBlips.blip.color, (Config.GPSBlips.blip.string):format(data.name), Config.GPSBlips.blip.scale, false)
            SetBlipCategory(7, blips[#blips])
        end
        ::skip::
        Wait(Config.GPSBlips.refreshrate or 3000)
        for k, data in pairs(blips) do
            RemoveBlip(data)
            blips[k] = nil
        end
    end
