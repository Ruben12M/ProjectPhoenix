			...
			if (selectedWindow == "none") {
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div class="ldata-txt">Carregar Dados...</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
				...
				setTimeout(function(){
					...
					$('#menu').html(`
						<div class="col-md-2 d-flex flex-column sidebar-s">
							...
							<div id="sidebar"></div>
							<p class="sidebar-item mt-auto logout"><i class="fas fa-sign-out-alt"></i></i> <span class="ms-1">Saír</span></p>
						</div>
...
function overview_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "bankmenu" || !event.data.isUpdate){
		...
		if(event.data.RequireCC) {
			informations = `<span class="card-o-title">Informações<span class="badge bg-primary viewall-badge buy_new_card" data-bs-toggle="modal" data-bs-target="#buycc_modal"><i class="fa-solid fa-plus"></i> NOVO</span></span>`;
		} else {
						...
						<div class="card-body ccard-body">
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
									<span class="d-flex justify-content-between"><span><img src="img/visa_white.svg" class="w20p"><span class="ccard-name">FLEECA Classic</span></span><span><i class="fas fa-wifi"></i></span></span>
									<div class="mt38p">
										...
										<div class="d-flex justify-content-between align-items-center">
											...
											<div class="d-flex align-items-center ccard-valid">
												<span class="ccard-thru">VÁLIDO ATÉ</span>
		...
		const months = ["Jan", "Fev", "Mar", "Abr", "Mai", "Junh", "Julh", "Agos", "Set", "Out", "Nov", "Dez"];
							...
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>A mudança de IBAN custou  ${event.data.ibanCost}€</span>
										<hr class="fff">
										<span>O IBAN tem sempre o prefixo "${event.data.ibanPrefix}"</span>
										<hr class="fff">
										<span>O número máximo de caracteres é ${event.data.ibanCharNum}</span>
									</div>
								</div>
							</div>
						</div>
					...
					<div>
						<div class="row">
							...
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Um novo PIN tem o custo de ${event.data.pinCost}€</span>
										<hr class="fff">
										<span>O número máximo de caracteres é ${event.data.pinCharNum}</span>
										<hr class="fff">
										<span>Apenas podes utilizar números</span>
									</div>
								</div>
							</div>
						</div>
	...
	setTimeout(function () {
		$('#menu').html(`
			<div class="d-flex justify-content-center flex-column align-items-center">
				<span class="load"></span>
				<br>
				<div class="ldata-txt">A Saír...</div>
			</div>
		`);
		$("#menu").fadeIn();
		setTimeout(function(){
